...
interface TraceLogPanelProps {
  /** The review_id or bulk_id to stream traces for */
  traceId: string
  /** SSE endpoint URL — e.g. "/api/reviews/{id}/traces" or "/api/bulk/traces/{id}" */
  endpoint: string
...
const PHASE_COLORS: Record<string, string> = {
  init: 'text-surface-400',
  prd_parse: 'text-primary-400',
  codebase_analysis: 'text-blue-400',
  llm_dispatch: 'text-accent-400',
...
export default function TraceLogPanel({
  ...
  // Connect to SSE (only re-creates when traceId/endpoint change, not isRunning)
  useEffect(() => {
        ...
        <div className="flex items-center gap-3">
          ...
          {!done && isRunning && (
            <span className="flex items-center gap-1 text-xs text-primary-400">
              <Circle className="w-2 h-2 fill-primary-400 animate-pulse" />
              Live
            </span>
          )}
          {done && (
        ...
        {!collapsed && (
          ...
          >
            ...
            {uniquePhases.length > 1 && (
              <div className="flex items-center gap-2 px-4 py-2 border-b border-surface-700/50 overflow-x-auto">
            ...
            >
              ...
              {filteredEvents.map((event, i) => {
                ...
                const levelColor = LEVEL_COLORS[event.level] || 'text-surface-400'
                const phaseColor = PHASE_COLORS[event.phase] || 'text-surface-400'
                const phaseLabel = PHASE_LABELS[event.phase] || event.phase.toUpperCase()

                return (
                  <div
                    key={i}
                    className={`flex items-start gap-2 px-4 py-1.5 hover:bg-surface-800/30 transition-colors border-b border-surface-800/30 ${
                      event.level === 'error'
                        ? 'bg-red-500/5'
                        : event.level === 'warn'
                        ? 'bg-yellow-500/5'
                  ...
                  >
                    {/* Elapsed time */}
                    <span className="text-surface-600 w-14 text-right flex-shrink-0 tabular-nums">
                    ...
                    {/* Phase badge */}
                    <span
                      className={`px-1.5 py-0 rounded text-[10px] font-bold flex-shrink-0 bg-surface-800 ${phaseColor}`}
                    >
                      {phaseLabel}
                    </span>
              ...
              {done && filteredEvents.length > 0 && (
                <div className="px-4 py-2 text-center text-xs text-surface-600 border-t border-surface-700/50">
