...
export default function Chat() {
  ...
  const indexCodebase = async () => {
    ...
    try {
      const res = await fetch('/api/chat/index-codebase', {
        method: 'POST',
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-surface-900/50 backdrop-blur-sm rounded-2xl border border-surface-800 p-6">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Enter path to your codebase or GitHub URL"
              className="w-full pl-12 pr-4 py-3 bg-surface-800 border border-surface-700 rounded-lg text-white placeholder-surface-500 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-all"
            />
          </div>
          <button
            onClick={indexCodebase}
            disabled={!codebasePath.trim() || isIndexing}
            className="flex items-center gap-2 px-6 py-3 bg-primary-500 text-white font-medium rounded-lg hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            {isIndexing ? (
              <>
        ...
        {codebaseInfo && (
          <motion.div
            ...
            className="mt-4 p-4 rounded-xl bg-primary-500/10 border border-primary-500/30"
          >
      ...
      {/* Chat area */}
      <div className="bg-surface-900/50 backdrop-blur-sm rounded-2xl border border-surface-800 flex flex-col" style={{ minHeight: '500px' }}>
        {/* Messages */}
                ...
                <div className="grid grid-cols-2 gap-3 max-w-lg mx-auto">
                  {starterQuestions.map(q => (
                    <button
                      ...
                      className="text-left text-sm px-4 py-3 rounded-xl bg-surface-800 text-surface-300 hover:bg-surface-700 hover:text-white transition-colors border border-surface-700"
                    >
                      {q}
                    </button>
                  ))}
            ...
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-xl px-4 py-3 text-sm ${
                ...
                {msg.citations && msg.citations.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-surface-700 space-y-1">
                    ...
                    {msg.citations.filter(c => c.type !== 'code').slice(0, 3).map((c, j) => (
                      <a key={j} href={c.url} className="flex items-center gap-1.5 text-xs text-primary-400 hover:text-primary-300">
                ...
                {msg.suggestedFollowups && msg.suggestedFollowups.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-surface-700 flex flex-wrap gap-2">
                    {msg.suggestedFollowups.map((q, j) => (
                      <button
                        key={j}
                        onClick={() => sendMessage(q)}
                        className="text-xs px-3 py-1.5 rounded-lg bg-surface-700/50 text-surface-300 hover:bg-surface-700 hover:text-white transition-colors"
                      >
                        {q}
                      </button>
                    ))}
        ...
        <div className="p-4 border-t border-surface-800">
          <div className="flex items-center gap-3">
            <input
              ...
              className="flex-1 bg-surface-800 border border-surface-700 rounded-lg px-4 py-3 text-white placeholder-surface-500 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500/30 disabled:opacity-50"
            />
            <button
              onClick={() => sendMessage()}
              disabled={!input.trim() || isLoading}
              className="p-3 rounded-lg bg-primary-500 hover:bg-primary-600 disabled:opacity-40 disabled:cursor-not-allowed text-white transition-colors"
            >
    ...
    <div className="space-y-2">
      {parts.map((part, i) => {
        if (part.startsWith('```') && part.endsWith('```')) {
          ...
          return (
            <pre key={i} className="bg-surface-900 border border-surface-700 rounded-lg p-3 overflow-x-auto text-xs">
