...
export default function CodebaseProfile() {
  ...
  const fetchProfiles = async () => {
    try {
      setLoading(true)
      const res = await fetch('/api/codebase-profiles')
      if (!res.ok) {
  ...
  const buildProfile = async () => {
    ...
    setError(null)
    try {
      const res = await fetch('/api/codebase-profiles/build', {
        method: 'POST',
  ...
  const deleteProfile = async (profileId: string) => {
    try {
      const res = await fetch(`/api/codebase-profiles/${profileId}`, { method: 'DELETE' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
  ...
  if (error && profiles.length === 0) {
    return (
      <div className="text-center py-12">
        <Shield className="w-10 h-10 text-surface-600 mx-auto mb-3" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold text-white">Codebase Profile</h1>
          <p className="text-surface-400 mt-1">Persistent security & health view across reviews</p>
        </div>
        <button
          onClick={() => setShowBuildForm(!showBuildForm)}
          className="flex items-center gap-2 px-4 py-2 bg-primary-500/20 text-primary-400 border border-primary-500/30 rounded-lg hover:bg-primary-500/30 transition-colors"
        >
      ...
      {showBuildForm && (
        <motion.div
          ...
          className="bg-surface-800/50 border border-surface-700 rounded-xl p-5"
        >
          <h3 className="text-sm font-semibold text-white mb-3">Build New Profile</h3>
          <div className="space-y-3">
            <div>
              <label className="text-xs text-surface-400 mb-1 block">Codebase Path</label>
              <input
                type="text"
                value={buildPath}
                onChange={e => setBuildPath(e.target.value)}
                placeholder="/path/to/codebase or github.com/owner/repo"
                className="w-full bg-surface-900 border border-surface-700 rounded-lg px-3 py-2 text-white text-sm placeholder-surface-500 focus:border-primary-500/50 focus:outline-none"
              />
            </div>
            <div>
              <label className="text-xs text-surface-400 mb-1 block">Display Name (optional)</label>
              <input
                type="text"
                value={buildName}
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-3">
          <div className="bg-surface-800/50 border border-surface-700 rounded-xl p-4">
            ...
            {profiles.length === 0 ? (
              <p className="text-xs text-surface-500">No profiles yet. Build one above.</p>
            ) : (
              <div className="space-y-2">
                {profiles.map(profile => (
                  ...
                  >
                    ...
                    <p className="text-xs text-surface-400 truncate mt-1">{profile.codebase_path}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-xs text-surface-500">{profile.review_count} reviews</span>
        ...
        <div className="col-span-9 space-y-6">
          {!p ? (
            <div className="text-center py-12">
              <Shield className="w-10 h-10 text-surface-600 mx-auto mb-3" />
              <p className="text-surface-400">Select or build a profile to view details</p>
            </div>
          ) : (
            <>
              {/* Overview cards */}
                ...
                {p.attack_surface.endpoints.length > 0 && (
                  <div className="space-y-1 max-h-60 overflow-y-auto">
                    {p.attack_surface.endpoints.map((ep, i) => (
                      <div key={i} className="flex items-center gap-3 px-3 py-2 bg-surface-800 rounded-lg border border-surface-700 text-xs">
                        <span className="font-mono text-surface-300 w-14 text-center px-1 py-0.5 bg-surface-700 rounded">{ep.method}</span>
                        <span className="text-surface-300 flex-1 truncate">{ep.path}</span>
                        {ep.has_auth ? (
                        ...
                        <span className="text-surface-500">{ep.review_count}x</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {Object.entries(p.entity_inventory.by_type).length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-4">
                    {Object.entries(p.entity_inventory.by_type).map(([type, count]) => (
                      <span key={type} className="text-xs px-2 py-1 bg-surface-700 text-surface-300 rounded-full capitalize">
                        {type}: {count}
                      </span>
                    ))}
                  </div>
                ...
                {p.entity_inventory.entities.length > 0 && (
                  <div className="space-y-1 max-h-60 overflow-y-auto">
                    {p.entity_inventory.entities.map((entity, i) => (
                      <div key={i} className="flex items-center gap-3 px-3 py-2 bg-surface-800 rounded-lg border border-surface-700 text-xs">
                        <span className="text-surface-300 flex-1">{entity.name}</span>
                        <span className="text-surface-500 capitalize">{entity.type}</span>
                        {entity.is_sensitive && (
                          <span className="px-1.5 py-0.5 bg-red-500/20 text-red-400 rounded text-[10px]">sensitive</span>
                        )}
                        <span className="text-surface-500">{entity.review_count}x</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              >
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-xs font-semibold text-surface-400 mb-3">By Dimension</h4>
                    <div className="space-y-2">
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-surface-400 mb-3">By Severity</h4>
                    <div className="space-y-2">
                ...
                {p.cumulative_findings.recurring_categories.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-xs font-semibold text-surface-400 mb-3">Recurring Categories</h4>
              ...
              >
                {p.historical_trend.reviews.length === 0 ? (
                  <p className="text-xs text-surface-500">No review history yet</p>
                ) : (
                  <div className="space-y-4">
                    <div className="flex items-end gap-2 h-32">
                      {p.historical_trend.reviews.map((point, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-1">
                          <span className="text-[10px] text-surface-400" style={{ color: RISK_COLORS[point.risk_rating] || '#6b7280' }}>
                            {point.risk_rating}
                          </span>
                          <motion.div
                            initial={{ height: 0 }}
                          ...
                          />
                          <span className="text-[10px] text-surface-500">{point.finding_count}</span>
                        </div>
                      ))}
                    </div>
                    <div className="space-y-1">
                      {p.historical_trend.reviews.slice().reverse().map((point, i) => (
                        <div key={i} className="flex items-center gap-3 text-xs px-3 py-2 bg-surface-800 rounded-lg border border-surface-700">
                          <span className="text-surface-500 w-24">{new Date(point.date).toLocaleDateString()}</span>
                          <span
                          ...
                          <span className="text-surface-400">{point.finding_count} findings</span>
                          {point.quality_score != null && (
                            <span className="text-surface-400 ml-auto">Quality: {point.quality_score}</span>
                          )}
                        </div>
                      ))}
                    </div>
...
function SectionCard({
  ...
  return (
    <div className="bg-surface-800/50 border border-surface-700 rounded-xl">
      <button
        onClick={onToggle}
