...
export default function PRDGenerator() {
  ...
  if (!isEnabled) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <FileText className="w-16 h-16 text-surface-500 mb-4" />
        ...
        <p className="text-surface-400 mb-6 text-center max-w-md">
          PRD Generator is not enabled. Set <code className="text-primary-400 bg-surface-800 px-2 py-0.5 rounded">FEATURE_PRD_GENERATOR=true</code> to enable.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      {/* Progress Steps */}
      <div className="flex items-center justify-between bg-surface-900/30 rounded-2xl p-2">
        {steps.map((s, index) => (
          <div key={s.id} className="flex items-center flex-1">
            ...
            {index < steps.length - 1 && (
              <ChevronRight className="w-5 h-5 text-surface-600 mx-1 flex-shrink-0" />
            )}
          </div>
        ))}
      </div>
...
function StepCodebase({
  ...
  const availableLanguages = [
    ...
    { id: 'kotlin', label: 'Kotlin', color: 'text-purple-400' },
  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-surface-400">Enter the path to the codebase you want to generate a PRD from</p>
      </div>

      <div>
        <label className="block text-sm font-medium text-surface-300 mb-2">Codebase Path</label>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-surface-800 border border-surface-700 rounded-xl text-white placeholder-surface-500 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 font-mono text-sm"
          />
        </div>
      ...
      <div>
        <label className="block text-sm font-medium text-surface-300 mb-3">Languages to Analyze</label>
      ...
      <div className="flex justify-end pt-4">
        <button
          ...
          className="px-6 py-3 bg-primary-500 text-white rounded-xl font-medium hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
        >
          Continue
  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-surface-400">Customize what should be included in the generated PRD</p>
      </div>

      {/* Focus Areas */}
      <div>
        <label className="block text-sm font-medium text-surface-300 mb-3">Focus Areas</label>
      ...
      {/* Detail Level */}
      <div>
        <label className="block text-sm font-medium text-surface-300 mb-3">Detail Level</label>
      ...
      {/* Include Options */}
      <div>
        <label className="block text-sm font-medium text-surface-300 mb-3">Include Sections</label>
      ...
      <div className="flex justify-between pt-4">
        <button
          onClick={onBack}
          disabled={isLoading}
          className="text-surface-400 hover:text-white transition-colors px-4 py-2"
        >
          Back
        </button>
        <button
          onClick={onNext}
          disabled={isLoading}
          className="px-6 py-3 bg-primary-500 text-white rounded-xl font-medium hover:bg-primary-600 disabled:opacity-50 transition-colors flex items-center gap-2"
        >
          {isLoading ? (
            <>
...
function ToggleOption({
  ...
  return (
    <label
      ...
      `}
    >
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${checked ? 'bg-primary-500/20' : 'bg-surface-800'}`}>
        <Icon className={`w-5 h-5 ${checked ? 'text-primary-400' : 'text-surface-500'}`} />
      </div>
...
function StepGenerating({
  ...
  return (
    ...
    >
      ...
      <p className="text-primary-400 mb-8">{currentStep}</p>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isCompleted ? 'text-primary-400' : isCurrent ? 'text-white bg-surface-800/50' : 'text-surface-500'}
              `}
            >
              {isCompleted ? (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-surface-400">{prd.summary}</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onStartNew}
            className="px-4 py-2 border border-surface-700 rounded-xl text-surface-300 hover:bg-surface-800 transition-colors flex items-center gap-2"
          >
            <RefreshCw className="w-4 h-4" />
            New
          </button>
          <div className="relative group">
            <button className="px-4 py-2 bg-primary-500 text-white rounded-xl font-medium hover:bg-primary-600 transition-colors flex items-center gap-2">
              <Download className="w-4 h-4" />
              Export
              <ChevronDown className="w-4 h-4" />
            </button>
            <div className="absolute right-0 top-full mt-2 w-48 bg-surface-800 border border-surface-700 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
              <button
                onClick={() => onExport('markdown')}
                disabled={isExporting}
                className="w-full px-4 py-3 text-left text-surface-300 hover:bg-surface-700 hover:text-white transition-colors flex items-center gap-2 rounded-t-xl"
              >
              ...
              <button
                ...
                disabled={isExporting}
                className="w-full px-4 py-3 text-left text-surface-300 hover:bg-surface-700 hover:text-white transition-colors flex items-center gap-2"
              >
              ...
              <button
                ...
                disabled={isExporting}
                className="w-full px-4 py-3 text-left text-surface-300 hover:bg-surface-700 hover:text-white transition-colors flex items-center gap-2 rounded-b-xl"
              >
      ...
      {prd.features.length > 0 && (
        <div className="p-6 rounded-xl bg-surface-800/30 border border-surface-700">
          ...
          <div className="grid grid-cols-2 gap-4">
            {prd.features.map((feature, idx) => (
              <div key={idx} className="p-4 rounded-lg bg-surface-900/50 border border-surface-700">
        ...
        <div className="p-6 rounded-xl bg-surface-800/30 border border-surface-700">
          ...
          <div className="flex flex-wrap gap-2">
            {prd.technical_stack.map((tech, idx) => (
              <span
                key={idx}
                className="px-3 py-1 rounded-lg bg-accent-500/10 text-accent-400 border border-accent-500/30 text-sm"
              >
                {tech}
              </span>
            ))}
              ...
              {isEditing ? (
                <div className="space-y-4">
                  <textarea
                    ...
                    rows={10}
                    className="w-full p-3 bg-surface-800 border border-surface-700 rounded-xl text-white font-mono text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary-500/50"
                  />
                ...
                <>
                  <div className="prose prose-invert max-w-none mb-4">
                    <pre className="whitespace-pre-wrap text-sm text-surface-300 font-sans">
                      {section.content}
                    </pre>
                  </div>
                  <div className="flex items-center gap-2 pt-4 border-t border-surface-700">
                    <button
                      onClick={onStartEdit}
                      className="flex items-center gap-2 px-3 py-1.5 text-sm text-surface-400 hover:text-white hover:bg-surface-800 rounded-lg transition-colors"
                    >
